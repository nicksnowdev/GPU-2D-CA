...
function windowResized() {
  resizeCanvas(floor(windowHeight * 0.8 * 0.5) * 2, floor(windowHeight * 0.8 * 0.5) * 2);
...
function zoomControl(event) {
  ...
  if (event.deltaY < 0) {
    zoom = min(16, zoom * 2);
    if (zoomCheck != zoom) {
      // only center the zoom if a zoom adjustment actually occurred
    ...
    if (zoomCheck != zoom) {
      // only center the zoom if a zoom adjustment actually occurred
      panX += ((mouseX - halfWidth) / zoom) * 0.5;
      panY += ((mouseY - halfHeight) / zoom) * 0.5;
    }
  }
  if (zoomCheck != zoom) {
...
function mouseDragged(event) {
  if (mouseButton == CENTER) {
    panX = min(
      max((-halfWidth / zoom) * (zoom - 1), panX + event.movementX / zoom),
      (halfWidth / zoom) * (zoom - 1)
    );
    panY = min(
      max((-halfHeight / zoom) * (zoom - 1), panY + event.movementY / zoom),
      (halfHeight / zoom) * (zoom - 1)
    );
  }
  return true;
}
...
function generate() {
  ...
  if (controls.symmetry) {
    // iterate through the array, get each one's 4, set all to same random number
    for (let i = 0; i < 512; i++) {
      if (ruleArray[i] == -1) {
    ...
    // iterate through the array, setting each index to random 0 or 1
    for (let i = 0; i < 512; i++) {
...
function setup() {
  ...
  canvas = createCanvas(
    floor(windowHeight * 0.8 * 0.5) * 2,
    floor(windowHeight * 0.8 * 0.5) * 2,
    WEBGL
  ); // 3D mode to allow shaders
  ...
  pauseBtn.on('click', () => {
    // alternatively, use () => yourFunc(anArg, anotherArg) to call any function with arguments
    if (!paused) {
      paused = true;
  ...
  ruleControls
    .addInput(controls, 'rule', {
      ...
      options: {
        ...
        nice_still_lifes:
          '',
        write_your_name:
          '',
        streaking_paint:
          '',
        thermal_vision:
          '',
        magma_cooling:
          '',
        division_decay:
          '',
        mandala_tiles_7:
          '',
        mandala_strobe:
          '',
        mandala_square_variety:
          '',
        mandala_pause_4:
          '',
        mandala_nice_pattern:
          '',
        sandy:
          '',
        desication:
          '',
        molten_crystals:
        ...
        concavity_battle:
          '',
        diamonds:
          '',
        squares:
        ...
        glider_entropy:
          '',
      },
    })
    .on('change', () => {
      ruleText.value(controls.rule);
    ...
    for (let i = 0; i < controls.iterations; i++) {
      ...
      if (!breaker && mousedOver) {
        ...
        if (painted) {
          ...
          noiseShader.setUniform('u_time', millis() * 0.001); // randomize the noise, lol
          graphics2.rect(-halfWidth, -halfHeight, width, height); // a container (the size of graphics1) to draw through the shader
          graphics2.pop(); // this resets the shader, otherwise need to call resetShader()
        } else {
          // also hide the brush outline while painting
          ...
          ellipse(
            (paintX - halfWidth + panX) * zoom,
            (paintY - halfHeight + panY) * zoom,
            controls.brushSize * zoom
          );
          pop();
        }
      }
